...
export async function authenticateAna(forceRefresh = false) {
  ...
  try {
    const response = await fetch('/api/ana/EstacoesTelemetricas/OAUth/v1', {
      method: 'GET',
      headers: {
...
export async function fetchHistoricoEstacao(codigoEstacao: number, rangeDias: 'DIAS_14' | 'DIAS_30' | 'DIAS_90' | 'DIAS_180' = 'DIAS_14', dataBusca?: string) {
  ...
  try {
    ...
    const response = await fetch(`/api/ana/EstacoesTelemetricas/HidroinfoanaSerieTelemetricaAdotada/v1?${params.toString()}`, {
      method: 'GET',
      headers: {
